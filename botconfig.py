...
IDENT = NICK
REALNAME = NICK
USERNAME = "Hoobshanker"  # For authentication; can be left blank if the same as NICK.
PASS = "password"  # can be None if authenticating with client certificates (see below)
PASS = "password"  # can be None if authenticating with client certificates (see below)
...
# openssl s_client -connect chat.freenode.net:6697 < /dev/null 2>/dev/null | openssl x509 -fingerprint
...
OWNERS = ("Hoobshanker",)  # The comma is required at the end if there is only one owner.
OWNERS_ACCOUNTS = ("Hoobshanker",)
...
ALLOWED_NORMAL_MODE_COMMANDS = ["Frole"]
