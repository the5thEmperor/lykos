...
def setup_variables(rolename):
    ...
    def shoot(var, wrapper, message):
        ...
        if check_protected(target):
            ...
            return
        target = src.status.try_misdirection(var, wrapper.source, target)

        if src.status.try_exchange(var, wrapper.source, target):
            return
        ...
        shoot_evt = Event(
            "gun_shoot", {"hit": rand <= gun_evt.data["hit"], "kill": random.random() <= gun_evt.data["headshot"]})
        ...
        if shoot_evt.data["hit"]:
            ...
            if realrole in Wolf:
                ...
                src.status.add_dying(var, target, killer_role=get_main_role(wrapper.source), reason="gunner_victim")
                if src.status.kill_players(var):
                    return
                ...
                src.status.add_absent(var, target, "wounded")
            ...
            src.status.add_dying(var, wrapper.source, killer_role="villager", reason="gunner_suicide")
            # blame explosion on villager's shoddy gun construction or something
            src.status.kill_players(var)

    @event_listener("transition_night_end", listener_id="<{}>.on_transition_night_end".format(rolename))
    def on_transition_night_end():
        for gunner in get_all_players((rolename,)):
            if GUNNERS[gunner]:
                gunner.send(messages["{0}_notify".format(rolename)].format(GUNNERS[gunner]))

    @event_listener(
        "transition_day_resolve_end", priority=4, listener_id="<{}>.on_transition_day_resolve_end".format(rolename))
    def on_transition_day_resolve_end(evt, var):
            ...
            if GUNNERS.get(victim) and "@wolves" in evt.data["killers"][victim]:
                if random.random() < var.GUNNER_KILLS_WOLF_AT_NIGHT_CHANCE:
                    ...
                    if wolfset:
                        ...
                        evt.data["message"][
                            victim].append(messages[to_send].format(victim, deadwolf, get_reveal_role(deadwolf)))
    ...
    def on_revealroles_role(evt, user, role):
